...
/**
 * Attendance module user-facing view actions.
 */
class View extends Base
{
	...
	public function dispAttendanceBoard(): void
	{
		...
		if ($config->greeting_list)
		{
			...
			$greeting_name = $greeting_list[mt_rand(0, count($greeting_list) - 1)];
		...
		$this->setTemplateFile($this->_resolveSkinTemplate('index'));
	}

	public function dispAttendanceBoardGiftList(): void
	{
		...
		$this->setTemplateFile($this->_resolveSkinTemplate('gift'));
	}

	public function dispAttendanceModifyContinuous(): void
	{
		$logged_info = \Context::get('logged_info');
	...
	public function dispAttendanceMemberInfo(): void
	{
		if (!\Context::get('is_logged'))
		{
		...
		$member_data->total_absent = $oAttendanceModel->getTotalAbsent($member_info->regdate, $total_attendance);
		$member_data->total_weekly = $oAttendanceModel->getWeeklyAttendanceByMemberSrl($member_srl);

		\Context::set('member_data', $member_data);
		$this->setTemplateFile($this->_resolveSkinTemplate('member_info'));
	}
}
