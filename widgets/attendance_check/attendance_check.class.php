...
class attendance_check extends WidgetHandler
{
	function proc($args)
	{
		...
		$continuity->about_continuity = $config->about_continuity;
		$continuity->continuity_day = $config->continuity_day;

		//지정일 포인트 설정
		$obj->about_target = $config->about_target;
		$obj->target_day = $config->target_day;

		//등수포인트 초기화
		$obj->add_1st = $config->first_point;
		$obj->add_2nd = $config->second_point;
		$obj->add_3rd = $config->third_point;
