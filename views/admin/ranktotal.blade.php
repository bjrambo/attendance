@include('_header')

@load('^/modules/attendance/tpl/filter/delete_all_data.xml')
@load('^/modules/attendance/tpl/filter/update_point.xml')
@load('^/modules/attendance/tpl/filter/fix_total_data.xml')

{!! $lang->attendance_total_help !!}<br />
<form action="{{ getUrl('') }}" method="POST" onsubmit="return procFilter(this, fix_total_data);" id="fixTotalData">
	<input type="hidden" name="module" value="{{ $module }}" />
	<input type="hidden" name="act" value="{{ $act }}" />
	<input type="hidden" name="type" value="{{ $type }}" />
	<input type="hidden" name="selected_date" value="{{ $selected_date }}" />
	<input type="hidden" name="member_srl" value="" />
	{{ $lang->attend_insert_continuity }}<input type="text" name="continuity" value="1" class="inputTypeText w60" />
</form>

<form action="{{ getUrl('') }}" method="get" class="adminSearch">
	<input type="hidden" name="error_return_url" value="" />
	<input type="hidden" name="mid" value="" />
	<input type="hidden" name="module" value="{{ $module }}" />
	<input type="hidden" name="act" value="{{ $act }}" />
	<input type="hidden" name="type" value="{{ $type }}" />
	<input type="hidden" name="selected_date" value="{{ $selected_date }}" />
	<fieldset>
		<select name="selected_group_srl">
			<option value="0">{{ $lang->group }}</option>
			@foreach($group_list as $key => $val)
			<option value="{{ $val->group_srl }}" @selected($selected_group_srl == $val->group_srl)>{{ $val->title }}</option>
			@endforeach
		</select>
		<select name="search_target">
			<option value="" @selected(!$search_target)>{{ $lang->search_target }}</option>
			<option value="user_id" @selected($search_target == 'user_id')>{{ $lang->user_id }}</option>
			<option value="nick_name" @selected($search_target == 'nick_name')>{{ $lang->nick_name }}</option>
			<option value="email_address" @selected($search_target == 'email_address')>{{ $lang->email }}</option>
		</select>
		<input type="text" name="search_keyword" value="{{ $search_keyword }}" class="inputTypeText" />
		<span class="button"><input type="submit" value="{{ $lang->cmd_search }}" /></span>
		<a href="#" onclick="location.href='{{ getUrl('','type',$type,'act',$act,'module',$module) }}';return false;" class="button"><span>{{ $lang->cmd_cancel }}</span></a>
	</fieldset>
</form>

<form action="{{ getUrl('') }}" method="POST" onsubmit="return procFilter(this, delete_all_data);" id="deleteAllData">
	<input type="hidden" name="module" value="{{ $module }}" />
	<input type="hidden" name="act" value="{{ $act }}" />
	<input type="hidden" name="type" value="{{ $type }}" />
	<input type="hidden" name="selected_date" value="{{ $selected_date }}" />
	<input type="hidden" name="member_srl" value="" />
</form>

<table cellspacing="0" class="attendanceTable">
	<tr>
		<th class="center" style="min-width:20px;">{{ $lang->position }}</th>
		<td class="center">{{ $lang->email }}</td>
		<th class="center">{{ $lang->nick_name }}</th>
		<th class="center">{{ $lang->view_total_day }}</th>
		<td class="center" style="width:270px;">{{ $lang->view_total_point }}</td>
		<th class="center" style="min-width:30px;">{{ $lang->attendance_perfectly_delete }}</th>
	</tr>
	@php $position = 1; @endphp
	@foreach($user_data->data as $ranking)
	@if($position == 1)@php $top = $ranking->total; @endphp@endif
	<tr>
		<th class="center">{{ $position }}</th>
		<td class="center">{{ $ranking->email_address }}</td>
		<th class="center">
			<div class="member_{{ $ranking->member_srl }}">{{ $ranking->nick_name }}</div>
		</th>
		<th class="center">
			<a href="#fixTotalData" onclick="att_fix_total_data('{{ $ranking->member_srl }}','{{ $lang->attend_fix_total_1 }}','{{ $lang->attend_fix_total_2 }}','{{ $ranking->nick_name }}');">{{ $ranking->total }}</a>
		</th>
		<td style="width:270px;">
			{{ $ranking->total_point }}
			<form action="./" method="get">
				<input type="text" id="point_{{ $ranking->member_srl }}" name="point" class="inputTypeText" style="width:50px;" />
				<span class="button small"><input type="button" value="+" onclick="updatePoint('{{ $ranking->member_srl }}', 'add')" /></span>
				<span class="button small"><input type="button" value="-" onclick="updatePoint('{{ $ranking->member_srl }}', 'minus')" /></span>
				<span class="button small"><input type="button" value="{{ $lang->cmd_update }}" onclick="updatePoint('{{ $ranking->member_srl }}', 'update')" /></span>
			</form>
		</td>
		<th class="center">
			<a href="#deleteAllData" onclick="att_delete_all_data('{{ $ranking->member_srl }}','{{ $ranking->nick_name }}');">×</a>
		</th>
	</tr>
	@php $position++; @endphp
	@endforeach
</table>

<form id="updateForm" action="{{ getUrl('') }}" method="POST">
	<input type="hidden" id="update_action" name="action" value="" />
	<input type="hidden" id="update_member_srl" name="member_srl" value="" />
	<input type="hidden" id="update_point" name="point" class="inputTypeText w80" />
</form>

<div class="pagination a1">
	<a href="{{ getUrl('page', $user_data->page_navigation->first_page, 'module_srl', '') }}" class="prevEnd">{{ $lang->first_page }}</a>
	@if($user_data->page_navigation)
	@while($page_no = $user_data->page_navigation->getNextPage())
	@if($user_data->page == $page_no)
	<strong>{{ $page_no }}</strong>
	@else
	<a href="{{ getUrl('page', $page_no, 'module_srl', '') }}">{{ $page_no }}</a>
	@endif
	@endwhile
	@else
	@php $page_no = 1; @endphp
	<strong>{{ $page_no }}</strong>
	@endif
	<a href="{{ getUrl('page', $user_data->page_navigation->last_page, 'module_srl', '') }}" class="nextEnd">{{ $lang->last_page }}</a>
</div>
