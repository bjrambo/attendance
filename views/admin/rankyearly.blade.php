...
<table cellspacing="0" class="rowTable attendanceTable">
	<tr>
		...
		<th width="40%" style="min-width:240px;" class="center">{{ $lang->view_yearly_day }}</th>
		<td class="center">{{ $lang->view_yearly_point }}</td>
	</tr>
	...
	<tr>
		...
		<th width="40%" style="min-width:240px;">
			<div class="admin_graph">
				@php $percent = (int)($ranking->yearly / $eoy * 100); @endphp
				...
				<div class="num" style="left:200px;width:100px;">
					<strong>{{ $ranking->yearly }}</strong>/{{ $eoy }}@if($percent < 50) ({{ $percent }}%)@endif
				</div>
			</div>
		</th>
		<td class="center">{{ $ranking->yearly_point }}</td>
	</tr>
	@php $position++; @endphp
	@endforeach
</table>
