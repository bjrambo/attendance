...
<div class="center">
	@php
	$monday = zDate(date('YmdHis', strtotime($week->monday)), 'Y-m-d');
	$sunday = zDate(date('YmdHis', strtotime($week->sunday)), 'Y-m-d');
	@endphp
	{{ $monday }} ~ {{ $sunday }}
</div>
<br />
...
<table cellspacing="0" class="rowTable attendanceTable">
	<tr>
		...
		<th style="width:300px;" class="center">{{ $lang->view_weekly_day }}</th>
		<td class="center">{{ $lang->view_weekly_point }}</td>
	</tr>
	...
	<tr>
		...
		<th style="width:300px;">
			<div class="admin_graph">
				@php $percent = (int)($ranking->weekly / 7 * 100); @endphp
				...
				<div class="num" style="left:230px;width:70px;">
					<strong>{{ $ranking->weekly }}</strong>/7@if($percent < 50) ({{ $percent }}%)@endif
				</div>
			</div>
		</th>
		<td class="center">{{ $ranking->weekly_point }}</td>
	</tr>
	@php $position++; @endphp
	@endforeach
</table>
