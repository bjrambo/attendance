...
class attendanceController extends attendance
{
	...
	function procAttendanceInsertAttendance()
	{
		...
		if(preg_match("/^\#/",$objs->greetings)) return new Object(-1, 'attend_greetings_error');

		if($obj->present_y == 'Y')
		{
			$giftMessage = sprintf('축하합니다! 이벤트에 당첨되었습니다. "%s" 선물을 받았습니다.', $config->giftname);
			$this->setMessage($giftMessage);
		}
		else
		{
